...
const generateNewMap = async function (options) {
  console.log('In generate ' + options)
  ...
  const stationsNames = [
    ...new Set(
      await getRandomStationsNames(
        targetStationsCount + targetStationsCount / 4 + 3
      )
    ),
  ]
  ...
  while (targetStationsCount - stations.length > 1) {
    ...
    //Stations loop
    while (
      targetStationsCount - stations.length > 0 &&
      distance(endPoint, lastStationPosition) < 300
    ) {
      //Get position of next station
      const stationPosition = createRandomPoint(width, height)
      ...
      //Else create new Station
        ...
        const station = createNewStation(stationPosition, stationsNames)
...
function createNewStation(position, stationsNames) {
  return {
    position: position,
    name: stationsNames.pop(),
  }
}
// x, y, name
